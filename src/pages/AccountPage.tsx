...
function Dashboard({ extSession }: DashboardProps) {
  ...
  const ta = translations[lang].account
    ...
    <div className="min-h-screen px-4 py-8 bg-[#F9FAFB]">
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10">
          ...
          >
            <LogOut className="w-4 h-4" /> {ta.signOut}
          </button>
        </div>
          ...
          <div className="mb-8 p-6 rounded-[24px] bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.05)] text-center">
            ...
            {linkingStatus === 'success' && (
              <div className="flex flex-col items-center gap-3 text-green-600">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-tight">{ta.linkSuccess}</p>
                  <p className="text-xs text-gray-400 font-medium mt-1">{ta.linkSuccessDesc}</p>
                </div>
                <Button onClick={() => window.close()} className="mt-2 h-10 px-6 text-[10px] font-black uppercase tracking-widest bg-gray-900 text-white rounded-lg">{ta.closeWindow}</Button>
              </div>
            )}
            {linkingStatus === 'error' && (
              <div className="flex flex-col items-center gap-3 text-red-500">
                ...
                <div>
                  <p className="text-sm font-black uppercase tracking-tight">{ta.linkFailed}</p>
                  <p className="text-xs text-red-400 font-medium mt-1">{linkingError}</p>
                </div>
                <Button onClick={linkExtension} className="mt-2 h-10 px-6 text-[10px] font-black uppercase tracking-widest bg-gray-900 text-white rounded-lg">{ta.tryAgain}</Button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs font-medium">{ta.memberSince} {new Date(user?.created_at ?? '').toLocaleDateString(lang === 'en' ? 'en-US' : lang, { month: 'long', year: 'numeric' })}</p>
          </div>
        </div>
        ...
        {isPastDue && (
          <div className="flex items-start gap-3 px-4 py-3 rounded-xl mb-4 bg-red-50 border border-red-100">
            ...
            <div className="flex-1">
              <p className="text-xs font-black text-red-500 uppercase tracking-wide">{ta.paymentFailed}</p>
              <p className="text-xs text-red-700/60 mt-0.5 font-medium">{ta.paymentFailedDesc}</p>
            </div>
        ...
        {isCancelled && periodEnd && (
          <div className="flex items-start gap-3 px-4 py-3 rounded-xl mb-4 bg-amber-50 border border-amber-100">
            ...
            <div className="flex-1">
              <p className="text-xs font-black text-amber-600 uppercase tracking-wide">{ta.cancelled}</p>
              <p className="text-xs text-amber-700/60 mt-0.5 font-medium">{ta.cancelledDesc.replace('{plan}', plan.label).replace('{date}', periodEnd ?? '')}</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
          ...
          <div className="bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] rounded-[32px] p-8 flex flex-col">
            <div className="flex items-start justify-between mb-8">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-300 mb-2">{ta.currentPlan}</p>
                ...
                {periodEnd && isPaid && (
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight flex items-center gap-1.5 mt-2">
                    {isCancelled
                      ? <><Calendar className="w-3 h-3" /> {ta.ends} {periodEnd}</>
                      : <><RefreshCw className="w-3 h-3" /> {intervalLabel} · {ta.renews} {periodEnd}</>
                    }
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider" style={{
                ...
                {isPastDue ? ta.pastDue : ta.active}
              </div>
            </div>
            ...
            <div className="mt-auto">
              {!isPaid ? (
                ...
                >
                  ...
                  {isCancelled ? ta.reactivate : ta.manageBilling}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] rounded-[32px] p-8 flex flex-col relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-300 mb-2">{ta.aiCredits}</p>
              <div className="flex items-baseline gap-1 mb-6">
                <span className="text-4xl font-black text-gray-900">{profile.credits_remaining ?? 0}</span>
                <span className="text-sm font-bold text-gray-300 uppercase tracking-widest">{ta.remaining}</span>
              </div>
        ...
        <div className="bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] rounded-[32px] p-8 mb-12">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-black text-gray-900 mb-0.5">{ta.chromeExtension}</p>
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight">
                    {linkingStatus === 'linking' ? ta.linking :
                     linkingStatus === 'success' ? ta.synced :
                     linkingStatus === 'error' ? ta.linkFailed :
                     ta.waiting}
                  </p>
                </div>
              </div>
            ...
            >
              {ta.open} <ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
          {linkingError && (
        ...
        {!isPaid && (
          <div ref={pricingRef} className="mt-48 pt-24 border-t-2 border-dashed border-gray-100 -mx-4 md:-mx-8 lg:-mx-12 xl:-mx-32 bg-white/80 pb-64">
            <div className="px-6 mb-20 text-center max-w-2xl mx-auto">
              <h3 className="text-4xl font-black text-gray-900 uppercase tracking-tight">{ta.levelUp}</h3>
